...
class ProcessWeightDB(object):
    ...
    def run(self):
        ...
        for file in self.db_file_list:
            ...
            #weight_db_logic = WeightDBUtilities.WeightDBEntryLogic(filename)
            #self.db_logic_dict[file] = []
        ...
        for db_logic in self.db_logic_dict.values():
            ...
            # update the weights_by_gene and genes_for_an_rsid 
            count_gene = 0
            num_gene = len(db_logic.weights_by_gene.keys())
            for gene in db_logic.weights_by_gene.keys():
                count_gene += 1 
                if count_gene % 150 == 0:
                    logging.info("Percentage of genes processed" + str(count_gene/num_gene))
        ...
        #                     print(rsid)
        #                     tmp_logic_object.weights_by_gene[gene][rsid] = db_logic.weights_by_gene[gene][rsid]
        #                     if rsid not in tmp_logic_object.genes_for_an_rsid.keys():
        #                         tmp_logic_object.genes_for_an_rsid[rsid] = []
        #                     tmp_logic_object.genes_for_an_rsid[rsid].append(gene)               
        #         else:
        #             tmp_logic_object.weights_by_gene[gene] = db_logic.weights_by_gene[gene]  
        #             for rsid in db_logic.weights_by_gene[gene].keys():
        #                 if rsid not in tmp_logic_object.genes_for_an_rsid.keys():
...
#===============================================================================
    ...
    def buildCovarianceEntries(self, name, gene, weight_db_logic, snps_by_rsid):
        ...
        cov_filename = "/ysm-gpfs/home/zy92/project/metaxcan/MetaXcan/software/covariance/results/" + gene + ".cov"
